...
from utils.env_wrappers2 import SubprocVecEnv, DummyVecEnv
import pdb
import random
...
def main():
    ...
    env_move_flag = 0  #when flag==1,the agents and landmarks move
    for j in range(num_updates):
        obs = envs.reset(env_move_flag)
        rollouts = []
        ...
        env_move_flag *= 0  #if env_move_flag==1,reset to 0
        ...
        #每100update后观察mean_reward是否大于阈值
        if j % 100 == 0 and rollouts[0].rewards.mean() >= 0.64:
            env_move_flag = 1
