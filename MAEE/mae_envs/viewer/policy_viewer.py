...
class PolicyViewer_hs(MjViewer):
    ...
    def run(self):
        ...
        while self.duration is None or time.time() < self.end_time:
            share_obs = torch.tensor(self.ob[1], dtype=torch.float).cuda().view(1,-1)
            if len(self.policies) == 0:
                ...
                for i, policy in enumerate(self.policies):
                    #inp = itemgetter(*ob_policy_idx[i])(self.ob)
                    #inp = listdict2dictnp([inp] if ob_policy_idx[i].shape[0] == 1 else inp)
                    obs = torch.tensor(self.ob[i+1], dtype=torch.float).cuda().view(1, -1)
                    value, action, _, recurrent_hidden_states = policy.act(share_obs, obs, 1, recurrent_hidden_states, masks)
