...
class MultiAgentEnv(gym.Env):
    ...
    def new_starts_obs(self, starts, now_agent_num, index_index, sample_index):
        # random.seed(seed)
        # rou_index = random.sample(range(len(starts)),1)[0]
        rou_index = sample_index[index_index] # seed代表starts的第几个,starts全部用来train
    ...
    def init_set(self, now_agent_num):
        for agent in self.agents:
            agent.state.p_pos = np.random.uniform(0, +1, self.world.dim_p)
            agent.state.p_vel = np.zeros(self.world.dim_p)
            agent.state.c = np.zeros(self.world.dim_c)
        for i, landmark in enumerate(self.world.landmarks):
            landmark.state.p_pos = np.random.uniform(0, +1, self.world.dim_p)
            landmark.state.p_vel = np.zeros(self.world.dim_p)
