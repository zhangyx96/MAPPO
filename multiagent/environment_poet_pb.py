...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
                 observation_callback=None, info_callback=None, share_callback=None,
    ...
    def init_set(self, now_agent_num, init_start):
        for i, agent in enumerate(self.agents):
            agent.state.p_pos = copy.deepcopy(init_start[i])
            agent.state.p_vel = np.zeros(self.world.dim_p)
            agent.state.c = np.zeros(self.world.dim_c)
        for i, landmark in enumerate(self.world.landmarks):
            landmark.state.p_pos = copy.deepcopy(init_start[i+now_agent_num])
            landmark.state.p_vel = np.zeros(self.world.dim_p) 
        self._reset_render()
        # record observations for each agent
        obs_n = []
