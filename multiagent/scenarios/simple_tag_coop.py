...
class Scenario(BaseScenario):
    ...
    def prey_policy(self, agent, world):
        ...
        x = length * np.cos(angle) * 5
        y = length * np.sin(angle) * 5
    ...
    def adversary_reward(self, agent, world):
        ...
        if agent.collide:
            for ag in agents:
                for adv in adversaries:
                    if self.is_collision(ag, adv) and ag.movable:
                        rew += 10
        return rew
    # def adversary_reward(self, agent, world):
    #     # Adversaries are rewarded for collisions with agents
        ...
        for other in world.agents:
            ...
            if other.adversary:
                ...
                if other.movable:
                    ...
                    good_alive.append(0)
        return np.concatenate([agent.state.p_vel] + [agent.state.p_pos] + adv_pos + good_pos + landmark_pos + [good_alive])
