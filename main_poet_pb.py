...
from a2c_ppo_acktr.model_poet_sp import Policy, ATTBase
from a2c_ppo_acktr.distributions import Bernoulli, Categorical, DiagGaussian
from a2c_ppo_acktr.storage import RolloutStorage
from evaluation import evaluate

from utils.make_env_poet_sp import make_env                         
from utils.env_wrappers_poet_sp import SubprocVecEnv, DummyVecEnv
import pdb
import random
import copy
...
def main():
    ...
    if args.share_policy:
        if args.use_attention:
            share_base = ATTBase(envs.observation_space[0].shape[0], args.adv_num)
    ...
    for j in range(args.num_processes):
        ...
        for indices in index_sample:
            for k in indices:
                one_starts_balls.append(copy.deepcopy(one_starts_landmark[k]+0.01))
                one_starts_agent.append(copy.deepcopy(one_starts_balls[0]+0.01))
                one_starts_agent.append(copy.deepcopy(one_starts_balls[0]-0.01))
        # pdb.set_trace()
        # select_starts.append(one_starts_agent+one_starts_landmark)
        starts_old.append(one_starts_agent + one_starts_balls + one_starts_landmark)
        print(starts_old)
        assert 0
        one_starts_agent = []
        one_starts_balls = []
